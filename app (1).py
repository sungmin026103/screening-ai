...
# ===========================================================================
    ...
    if mode.startswith("R 결과"):
        ...
        if es_file:
            ...
            mapping_incomplete = not (guess["study"] and guess["yi"] and (guess["vi"] or (guess["ci_lo"] and guess["ci_hi"])))

            with st.expander("열 매핑 확인 / 수정 (자동 인식이 틀렸을 때만 여세요)", expanded=mapping_incomplete):
                r1, r2 = st.columns(2)
                with r1:
            ...
            summary_ok = summary_file is not None and (sg["g"] != 0.0 or sg["ci_lb"] != 0.0 or sg["ci_ub"] != 0.0)
            with st.expander("종합효과 값 확인 / 수정", expanded=not summary_ok):
                if not summary_file:
                    st.caption("요약 CSV가 없으면 아래 값을 직접 입력하세요.")
            ...
            # 열/요약 값이 확인되는 즉시 자동으로 그림을 생성 — 버튼을 따로 누를 필요 없음
            try:
                ...
                if sub.empty:
                    ...
                    st.session_state["meta_r_result"] = None
                else:
    ...
    # -------------------------------------------------------------------
        ...
        if meta_file:
            ...
            essential_ok = all(guess[k] for k in
                ["study", "mean_treat", "sd_treat", "n_treat", "mean_control", "sd_control", "n_control"])
            detected_bits = [
                f"연구명={guess['study'] or '?'}",
                f"실험군(M/SD/N)={guess['mean_treat'] or '?'}/{guess['sd_treat'] or '?'}/{guess['n_treat'] or '?'}",
                f"대조군(M/SD/N)={guess['mean_control'] or '?'}/{guess['sd_control'] or '?'}/{guess['n_control'] or '?'}",
            ]
            if guess["subgroup"]:
                detected_bits.append(f"하위그룹={guess['subgroup']}")
            st.caption(("🔎 자동 인식: " if essential_ok else "⚠️ 일부 열을 자동으로 찾지 못해 아래에서 직접 확인이 필요합니다 — ")
                       + " · ".join(detected_bits))

            with st.expander("열 매핑 확인 / 수정 (자동 인식이 틀렸을 때만 여세요)", expanded=not essential_ok):
                r1c1, r1c2 = st.columns(2)
                with r1c1:
                    study_col = st.selectbox("연구명 열", cols, index=_idx(guess["study"], 0))
                with r1c2:
                    subgroup_options = ["(없음)"] + cols
                    default_sub = subgroup_options.index(guess["subgroup"]) if guess["subgroup"] in cols else 0
                    subgroup_pick = st.selectbox("하위그룹 열 (선택)", subgroup_options, index=default_sub)
                    subgroup_col = None if subgroup_pick == "(없음)" else subgroup_pick

                st.markdown("**실험군**")
                e1, e2, e3 = st.columns(3)
                with e1:
                    mean_t_col = st.selectbox("평균", cols, index=_idx(guess["mean_treat"], 1), key="mean_t")
                with e2:
                    sd_t_col = st.selectbox("SD", cols, index=_idx(guess["sd_treat"], 2), key="sd_t")
                with e3:
                    n_t_col = st.selectbox("N", cols, index=_idx(guess["n_treat"], 3), key="n_t")

                st.markdown("**대조군**")
                c1x, c2x, c3x = st.columns(3)
                with c1x:
                    mean_c_col = st.selectbox("평균", cols, index=_idx(guess["mean_control"], 4), key="mean_c")
                with c2x:
                    sd_c_col = st.selectbox("SD", cols, index=_idx(guess["sd_control"], 5), key="sd_c")
                with c3x:
                    n_c_col = st.selectbox("N", cols, index=_idx(guess["n_control"], 6), key="n_c")

            # 열 매핑이 확인되는 즉시 자동으로 계산 — 버튼을 따로 누를 필요 없음
            default_title = (guess["mean_treat"].split("_")[0] if guess.get("mean_treat") else "Effects of intervention")
            title_input_raw = st.text_input("그림 제목", value=default_title, key="raw_title")
            try:
        ...
        if meta_raw:
            ...
            # 다운로드용 고화질 PNG (Mode A와 동일한 리뷰어용 스타일)
            st.markdown('<div class="section-title" style="margin-top:18px;">다운로드용 그림</div>', unsafe_allow_html=True)
            title_r2 = meta_raw.get("title") or "Effects of intervention"
            sub_r2 = eff.rename(columns={
                "mean_t": "mean_treat", "sd_t": "sd_treat",
                "mean_c": "mean_control", "sd_c": "sd_control",
                "ci_low": "ci_lo", "ci_high": "ci_hi",
            })
            summary_r2 = ForestSummary(g=pooled.beta, ci_lb=pooled.ci[0], ci_ub=pooled.ci[1],
            ...
            if pooled.k >= 3 and not pd.isna(pooled.prediction_interval[0]):
                summary_r2.pi_lb, summary_r2.pi_ub = pooled.prediction_interval
            fig_f2 = forest_plot_from_R(sub_r2, summary_r2, title=title_r2)
            st.pyplot(fig_f2, use_container_width=True)
            dpi_pick2 = st.select_slider("다운로드 해상도 (DPI)", options=[150, 300, 600, 1200], value=300, key="raw_forest_dpi")
            st.download_button(
                f"Forest plot PNG 다운로드 ({dpi_pick2}dpi)", fig_to_png_bytes(fig_f2, dpi=dpi_pick2),
                f"forest_{title_r2.replace(' ', '_')}.png", "image/png", type="primary", use_container_width=True, key="raw_forest_dl",
            )
            fig_fn2 = funnel_plot_from_R(sub_r2, summary_r2, egger, title=title_r2)
            st.pyplot(fig_fn2, use_container_width=True)
            st.download_button(
                "Funnel plot PNG 다운로드", fig_to_png_bytes(fig_fn2, dpi=300),
                f"funnel_{title_r2.replace(' ', '_')}.png", "image/png", use_container_width=True, key="raw_funnel_dl",
            )
    ...
    # -------------------------------------------------------------------
        ...
        if meta_file:
            ...
            essential_ok = all(guess[k] for k in ["study", "yi"]) and (guess["ci_lo"] and guess["ci_hi"])
            st.caption(("🔎 자동 인식: " if essential_ok else "⚠️ 일부 열을 자동으로 찾지 못해 아래에서 직접 확인이 필요합니다 — ")
                       + f"연구명={guess['study'] or '?'} · 효과크기={guess['yi'] or '?'} · "
                       f"CI={guess['ci_lo'] or '?'}/{guess['ci_hi'] or '?'}")
            ...
            with st.expander("열 매핑 확인 / 수정 (자동 인식이 틀렸을 때만 여세요)", expanded=not essential_ok):
                ...
                with c1:
                    study_col = st.selectbox("연구명 열", cols, index=_idx(guess["study"], 0))
                with c2:
                    effect_col = st.selectbox("효과크기 열", cols, index=_idx(guess["yi"], 1))
                with c3:
                    ci_low_col = st.selectbox("CI 하한 열", cols, index=_idx(guess["ci_lo"], 2))
                with c4:
                    ci_high_col = st.selectbox("CI 상한 열", cols, index=_idx(guess["ci_hi"], 3))
