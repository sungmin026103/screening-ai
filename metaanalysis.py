...
def forest_plot_from_R(
    ...
    if summary.i2 is not None and not pd.isna(summary.i2):
        het_line = f"Heterogeneity: I² = {summary.i2:.1f}%"
    if has_pi:
        pi_txt = f"95% PI: [{summary.pi_lb:.2f}, {summary.pi_ub:.2f}]"
...
def baujat_plot(effect_df: pd.DataFrame, pooled: "PooledResult", cluster_col: str = "study",
                title: str = "Baujat plot", top_n: int = 3) -> "plt.Figure":
    yi = effect_df["yi"].to_numpy()
    vi = effect_df["vi"].to_numpy()
    wi_fixed = 1 / vi
    fixed_mean = float(np.sum(wi_fixed * yi) / np.sum(wi_fixed))
    ...
    studies = effect_df[cluster_col].astype(str).to_numpy()

    # 두 축을 0~1로 정규화한 뒤 합산 점수로 "가장 많이 기여하는" 연구를 선정
    def _norm(a):
        rng = a.max() - a.min()
        return (a - a.min()) / rng if rng > 1e-12 else np.zeros_like(a)
    score = _norm(contrib_q) + _norm(influence)
    ...
    if flagged:
        ...
        ax.scatter(contrib_q[flagged_idx], influence[flagged_idx], s=80, color=C_PI, zorder=4,
                  edgecolor="white", linewidth=0.7)
        for i in flagged_idx:
            ax.annotate(studies[i], (contrib_q[i], influence[i]), textcoords="offset points",
                       xytext=(6, 5), fontsize=9, color=C_PI, fontweight="bold")
