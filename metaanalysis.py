...
_COLUMN_ALIASES = {
    ...
    "n_treat": ["n_treat", "ntreat", "n1", "nexperimental", "n_exp", "experimentaln", "ne", "nt", "실험군n"],
    "mean_treat": ["mean_treat", "meantreat", "m1", "mean1", "meanexperimental", "experimentalmean",
                   "meane", "meant", "meangroup1", "실험군평균"],
    "sd_treat": ["sd_treat", "sdtreat", "sd1", "sdexperimental", "experimentalsd", "sde", "sdt", "실험군sd"],
    "n_control": ["n_control", "ncontrol", "n2", "controln", "nc", "대조군n"],
    "mean_control": ["mean_control", "meancontrol", "m2", "mean2", "controlmean", "meanc", "meangroup2", "대조군평균"],
    "sd_control": ["sd_control", "sdcontrol", "sd2", "controlsd", "sdc", "대조군sd"],
...
def forest_plot_from_R(
    ...
    Y_TICKNUM = Y_AXIS_BASE - 0.20
    Y_SUBTITLE = Y_AXIS_BASE - 0.60
    Y_MIN = -1.85
    Y_MAX = Y_TITLE + 0.35
    ...
    ax.plot([xf(0), xf(0)], [Y_AXIS_BASE, Y_SEP1], color=C_ZERO, lw=0.9, ls="--", zorder=1)
...
def funnel_plot_from_R(sub: pd.DataFrame, summary: ForestSummary, egger: EggerResult | None = None,
    ...
    fig, ax = plt.subplots(figsize=(7.6, 6.2), dpi=100)
...
# ---------------------------------------------------------------------------
# 7. 민감도/진단 그림 — Leave-one-out, Baujat, GOSH, Trim-and-fill, Influence
#    (모두 pool_random_effects()를 반복 재적합해 계산하는 근사치이며,
#     R metafor의 정확한 3-level CRVE 알고리즘과 100% 동일하지는 않다.)
# ---------------------------------------------------------------------------
...
def leave_one_out_plot(effect_df: pd.DataFrame, pooled: "PooledResult", cluster_col: str = "study",
    ...
    k = len(loo)
    y = np.arange(k, 0, -1)
    fig, ax = plt.subplots(figsize=(7.6, max(3.2, 0.34 * k + 1.4)), dpi=100)
...
def baujat_plot(effect_df: pd.DataFrame, pooled: "PooledResult", cluster_col: str = "study",
                title: str = "Baujat plot") -> "plt.Figure":
    yi = effect_df["yi"].to_numpy()
    vi = effect_df["vi"].to_numpy()
    wi_fixed = 1 / vi
    fixed_mean = float(np.sum(wi_fixed * yi) / np.sum(wi_fixed))
    contrib_q = wi_fixed * (yi - fixed_mean) ** 2  # 개별 연구의 이질성 기여도 (x축)

    loo = leave_one_out(effect_df, cluster_col)
    influence = ((pooled.beta - loo["g"].to_numpy()) ** 2) / (pooled.se ** 2)  # 영향력 (y축)

    fig, ax = plt.subplots(figsize=(6.8, 5.8), dpi=100)
    ax.scatter(contrib_q, influence, s=64, color=C_NORM, zorder=3, edgecolor="white", linewidth=0.6)
    for x, y, label in zip(contrib_q, influence, effect_df[cluster_col].astype(str)):
        ax.annotate(label, (x, y), textcoords="offset points", xytext=(5, 4), fontsize=8, color="#444444")
...
def gosh_plot(effect_df: pd.DataFrame, n_iter: int = 1500, min_frac: float = 0.5, seed: int = 42,
             title: str = "GOSH plot") -> "plt.Figure":
    k = len(effect_df)
    if k < 4:
        raise ValueError("GOSH plot에는 최소 4개 이상의 연구가 필요합니다.")
    rng = np.random.default_rng(seed)
    idx = np.arange(k)
    gs, i2s = [], []
    for _ in range(n_iter):
        m = int(rng.integers(max(3, int(k * min_frac)), k))
        sub_idx = rng.choice(idx, size=m, replace=False)
        try:
            p = pool_random_effects(effect_df.iloc[sub_idx])
            gs.append(p.beta)
            i2s.append(p.i2)
        except Exception:
            continue
    ...
    ax.set_xlabel("Hedges' g (subset)")
    ax.set_ylabel("I² (%) (subset)")
    ax.set_title(title, fontsize=13, loc="left", fontweight="bold")
...
def trim_fill_plot(tf_result: dict, title: str = "Trim-and-fill sensitivity") -> "plt.Figure":
    ...
    fig, ax = plt.subplots(figsize=(7.4, 2.7), dpi=100)
    y = 1
    ...
    ax.scatter([orig.beta], [y + 0.14], s=100, color=C_POOL, marker="o", zorder=4,
              edgecolor="white", linewidth=0.8, label="Original g")
    ax.plot([adj.ci[0], adj.ci[1]], [y - 0.14] * 2, color=C_PI, lw=1.7, zorder=2)
    ax.scatter([adj.beta], [y - 0.14], s=120, color=C_PI, marker="D", zorder=4,
              edgecolor="white", linewidth=0.8, label="Trim-fill adjusted g")
...
def influence_plot(effect_df: pd.DataFrame, pooled: "PooledResult", cluster_col: str = "study",
    ...
    cooks = dffits ** 2
    thr_cook = float(np.mean(cooks) + 2 * np.std(cooks)) if k > 1 else 0.0
    thr_dffits = float(2 * np.sqrt(1 / max(k, 1)))
