...
def train_and_predict(df: pd.DataFrame, target_recall: float = 0.95) -> ScreeningResult:
    ...
    # The full pipeline (TF-IDF vectorizers + classifier) is refit from
    # scratch inside each CV fold via cross_val_predict, so validation-fold
    # text never leaks into a vectorizer's vocabulary/IDF statistics.
    probs = cross_val_predict(_build_pipeline(), texts, y, cv=cv, method="predict_proba")[:, 1]
